...
_file = new File(imc0_CONFIG_LOCALIZATION);
	if (_file->findFileLine("\n" imc0_LOCALIZATION) > 0)
		_file->deleteFileLine("\n" imc0_LOCALIZATION, _file->findFileLine("\n" imc0_LOCALIZATION));

 

_file = new File(gro0_CONFIG_LOCALIZATION);
	if (_file->findFileLine("\n" gro0_LOCALIZATION) > 0)
		_file->deleteFileLine("\n" gro0_LOCALIZATION, _file->findFileLine("\n" gro0_LOCALIZATION));

	 
	
	_file = new File(grw0_CONFIG_LOCALIZATION);
	if (_file->findFileLine("\n" grw0_LOCALIZATION) > 0)
		_file->deleteFileLine("\n" grw0_LOCALIZATION, _file->findFileLine("\n" grw0_LOCALIZATION));
...
_file = new File(gro0_CONFIG_LOCALIZATION);
	...
	_file = new File(imc0_LOCALIZATION);
	...
	if ( !_file->checkFileExist() ) {
		...
		//copy imc0 plugin
		_file = new File(imc0_APP_LOCALIZATION);

		if (_file->checkFileExist()) {
			_result = _file->copyFile(imc0_LOCALIZATION, NULL);
			if (!_result)
				return 0;
		}
		else {
	...
	_file->addFileLine("\n" imc0_LOCALIZATION, _file->findFileLine("*KERNEL"));

	_setup = imc0;
	return 1;
}
int Engine::switch_to_grw0() {
	_file = new File(UX0_CONFIG_LOCALIZATION);
	if (_file->findFileLine("\n" UX0_LOCALIZATION) > 0)
		_file->deleteFileLine("\n" UX0_LOCALIZATION, _file->findFileLine("\n" UX0_LOCALIZATION));
...
_file = new File(gro0_CONFIG_LOCALIZATION);
	...
	_file = new File(grw0_LOCALIZATION);
	...
	if ( !_file->checkFileExist() ) {
		...
		//copy grw0 plugin
		_file = new File(grw0_APP_LOCALIZATION);

		if (_file->checkFileExist()) {
			_result = _file->copyFile(grw0_LOCALIZATION, NULL);
			if (!_result)
				return 0;
		}
		else {
	...
	_file->addFileLine("\n" grw0_LOCALIZATION, _file->findFileLine("*KERNEL"));

	_setup = grw0;
	return 1;
}
int Engine::switch_to_gro0() {
	_file = new File(UX0_CONFIG_LOCALIZATION);
	if (_file->findFileLine("\n" UX0_LOCALIZATION) > 0)
		_file->deleteFileLine("\n" UX0_LOCALIZATION, _file->findFileLine("\n" UX0_LOCALIZATION));
...
_file = new File(grw0_CONFIG_LOCALIZATION);
	...
	_file = new File(gro0_LOCALIZATION);
	...
	if ( !_file->checkFileExist() ) {
		...
		//copy gro0 plugin
		_file = new File(gro0_APP_LOCALIZATION);

		if (_file->checkFileExist()) {
			_result = _file->copyFile(gro0_LOCALIZATION, NULL);
			if (!_result)
				return 0;
		}
		else {
	...
	_file->addFileLine("\n" gro0_LOCALIZATION, _file->findFileLine("*KERNEL"));
